...
def main():
	...
	for (l,lamb) in enumerate(lambs):
		...
		axs[l].scatter(t_axis[t_inds], rf_ref[t_inds], c='crimson', ec='k', s=40)
		axs[l].scatter(t_axis[t_inds], rf_pert[t_inds], c='steelblue', ec='k', s=40)
	...
	if save_fig:
		plt.savefig("ot_match_{}.png".format(int(lamb)))
		plt.savefig("ot_match_{}.pdf".format(int(lamb)))
	plt.show()
