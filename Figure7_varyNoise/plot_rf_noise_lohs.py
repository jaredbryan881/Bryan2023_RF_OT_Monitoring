...
def main():
	...
	for (n,noise_level) in enumerate(noise_levels[::-1]):
		...
		for i in range(n_rfs):
			...
			rfs_ref[i] = simulate_RF(ref_model, slow, baz, npts, dt, freq=flim, vels=None, noise=None).data + noise
		...
		for i in range(n_rfs):
			noise = np.random.normal(loc=0.0, scale=sigma, size=npts)
			rfs_pert[i] = simulate_RF(pert_model, slow, baz, npts, dt, freq=flim, vels=None, noise=None).data + noise

		# calculate grid
		G = create_grid(rfs_ref*1.1, t_axis, tlim, n_amp)
